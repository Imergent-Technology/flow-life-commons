...
`X-Powered-By` must not be sent: it names a PHP patch level to look up and tells a user nothing. `expose_php` is a php.ini-only setting — no `.user.ini`, no `ini_set()` override — and this hosting account's control panel exposes neither a MultiPHP INI Editor nor any other way to turn it off, so the ini value itself is not the enforcement point. **Measured directly on the production host during the first deployment rehearsal (2026-09-22, real Apache + CloudLinux LSAPI, release v0.1.0): every PHP response carried `X-Powered-By: PHP/8.3.33`**, superseding the 2026-09-21 probe's "no `X-Powered-By` header reaches clients" finding, which was wrong. The enforced invariant is now stated where it can actually be enforced — Apache — not PHP: `public/.htaccess` carries `Header onsuccess unset X-Powered-By` **and** `Header always unset X-Powered-By`, both, because LSAPI has been observed populating either of Apache's two response-header tables depending on the response class. **Re-verified on the same production host the same day, after deploying the fix as v0.1.1:** `X-Powered-By` was absent from `/`, `/api/v1/health` and `/up`, under maintenance and live. The CLI's own `expose_php` reading remains informational only (`security:production-check`; [production readiness](../runbooks/production-readiness.md)) — it may read `On` on this host indefinitely, and that is not a finding as long as the header itself never reaches a client.
