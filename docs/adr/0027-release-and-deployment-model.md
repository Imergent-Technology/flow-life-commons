...
**Probed individually on the host; composed and proved at two further levels since, neither of which is the host.** Each rewrite rule above was tested on its own on the real account, one rule at a time — the "`/api/*` never intercepted" row above is that individual maintenance-rule probe, not a probe of the composed file's full rewrite chain. The composed file now exists — header block, private-path denials, maintenance arm, `/api` and `/up` carve-out and SPA fallback, in that order — and the whole *contract* is driven in a real browser against Caddy, the production-equivalent development gateway, with the file's structure and rule order pinned by a test. That is proof of the **contract**, not of Apache: there is no Apache in that origin.
