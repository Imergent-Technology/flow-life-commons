...
- **Clarified:** 2026-09-23, after the backend was built. The decision is unchanged. `membership_grants`, the `Membership` module (`Domain`, `Application`, `Infrastructure`), the temporal derivation, race-safe one-way revocation, `Identity\Application\RegisterPerson`, and the `membership.records.view`/`membership.records.manage` capabilities now exist and are covered by tests: the temporal derivation by pure unit tests (no database involved), persistence and revocation by feature tests that run against both MariaDB and PostgreSQL, and the conditional-update revocation by a two-process concurrency proof. An operator-only `/admin/members` HTTP surface (Work Package 5) and a Guardian Console Membership administration UI on top of it (Work Package 6) are also now built. **Not built:** a member-facing API, WordPress integration, service/delegated authentication, and Zeffy/Luma automation. The requirement that an open-ended grant be an explicit operator choice is enforced at the HTTP boundary by a required `open_ended` boolean that must agree with `ends_at` (`true` with a null end, or `false` with a real end): a blank or missing end date is refused rather than read as open-ended, because the framework's empty-string-to-null conversion would otherwise make the two indistinguishable.
