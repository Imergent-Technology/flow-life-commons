...
| [0028](0028-membership-grants-derived-at-query-time.md) | Membership access is time-bounded grants, derived at query time | Accepted (design) |
| [0029](0029-commerce-providers-own-payment-facts.md) | Commerce providers own payment facts; Commons owns organizational entitlement | Accepted (design) |
...
ADRs 0028 and 0029 are the **Membership Foundation** design gate, and the first decisions about a business domain rather than about the platform that carries it. They remain **accepted design, mostly not implemented**: no `Membership` module, no `membership_grants` table and no membership capabilities exist yet. One piece Identity owns ahead of Membership is built: `Identity\Application\RegisterPerson`, which ADR 0028 places in Identity so a Person can be registered with no Account. [ADR 0028](0028-membership-grants-derived-at-query-time.md) decides how membership access is represented and why it is derived rather than stored; [ADR 0029](0029-commerce-providers-own-payment-facts.md) decides where the boundary sits between an external payment provider's facts and Commons' own interpretation of them.
