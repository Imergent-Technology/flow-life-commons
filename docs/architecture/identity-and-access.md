...
- **Known limits.**
  - A committed reset does not reach a session the transport establishes a moment later: the challenge commits before the session is created, so a reset landing in that window finds no session to end. It needs an attacker holding the password *and* a working authenticator to be finishing a sign-in at the instant an operator resets it. Closing it would mean holding the Account lock across session establishment, which the framework's session lifecycle makes invasive.
  ...
  - Nothing rate limits how often an operator may reissue an invitation; each is audited and follows a fresh, authorized proof.
