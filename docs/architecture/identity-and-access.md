...
- **The session middleware is a named `stateful` group applied to the Console's routes, not to the whole `api` group.** ADR 0016 says session middleware is added "explicitly to the API group". Applied literally, every API request would start a session, write a `sessions` row and set a cookie (including the public health endpoint), and every `POST` would need a CSRF token, which would contradict service clients that authenticate with a credential rather than a cookie ([ADR 0018](../adr/0018-client-and-delegated-authentication.md)). So the API stays stateless and cookie-authenticated routes join `stateful` explicitly with `->middleware('stateful')`. The group holds the same four pieces the ADR names. A test asserts that stateless endpoints get no cookie and no session row. *This is an interpretation of the ADR's wording; it is flagged for review.*
- **`PreventRequestForgery` is the CSRF middleware.** In Laravel 13 `ValidateCsrfToken` is a deprecated alias of it. It validates the `X-XSRF-TOKEN` header against the session token, **and also accepts a request the browser marks `Sec-Fetch-Site: same-origin`** (a header page scripts cannot set). A sibling host such as WordPress arrives as `same-site` and is refused unless it carries a valid token. Laravel skips CSRF entirely under PHPUnit, so the tests switch the environment to make it real.
