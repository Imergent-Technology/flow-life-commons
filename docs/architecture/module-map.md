...
The Membership Foundation design gate is complete ([ADR 0028](../adr/0028-membership-grants-derived-at-query-time.md), [ADR 0029](../adr/0029-commerce-providers-own-payment-facts.md)) and its backend and operator-only administration API now exist (above, "Current modules"; Work Package 5). **Not built:** the Guardian Console Membership UI, a member-facing API, WordPress integration, service/delegated authentication, and Zeffy/Luma automation.
