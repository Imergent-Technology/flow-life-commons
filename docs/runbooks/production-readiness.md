...
| 2 | The PHP version is not advertised to clients | **Regressed on the real host 2026-09-22; fixed in the repository, redeployment and re-verification still needed** | The 2026-09-21 CLI-only probe read `expose_php` as **On** and (wrongly) concluded no `X-Powered-By` header reached clients. The first deployment rehearsal (2026-09-22, real Apache + CloudLinux LSAPI) showed otherwise: **every PHP response carried `X-Powered-By: PHP/8.3.33`** — the maintenance responder and Laravel's own JSON/HTML responses alike. The hosting account's "Select PHP Version" UI has no `expose_php` toggle and there is no MultiPHP INI Editor for this account, so the ini setting cannot be changed from the host. `public/.htaccess` now strips the header at the origin instead (`Header onsuccess unset X-Powered-By` and `Header always unset X-Powered-By` — both, because LSAPI was observed populating either of Apache's two response-header tables); a real-Apache container test (`scripts/tests/apache-surface.sh`) proves this against `expose_php=On`. **This has not yet been re-verified on the production host itself** — that is the next thing to confirm once the fixed release is deployed. `expose_php` reading `On` in the CLI or web ini is not itself a finding; the client-visible header is. |
