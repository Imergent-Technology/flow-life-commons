...
**Proven locally as a whole; the real file has still never served a release.** The composed contract — security headers, private-path denials, the maintenance arm, the API and `/up` carve-out and the SPA fallback, in that order — is now exercised end to end in a real browser against the production-equivalent origin (`apps/guardian-console/e2e/production-surface.spec.ts`), and the committed `.htaccess` is pinned structurally, order included, by `ProductionSurfaceTest`. That proves the **contract**. It does not execute Apache: there is no Apache in development, so the `.htaccess` itself is still evidenced only by the per-rule probes on the host (above) plus those structural pins. The first real deployment is what exercises the file.
