...
**Not yet proven, and not claimed:** the rules above were tested **in isolation**. The real `.htaccess` must carry the generated header block, the maintenance arm, the `/api` and `/up` carve-out, the SPA fallback and the private-path denials **in one file, in the right order**. Rule ordering is where such a file goes wrong, and the composed set is implementation-phase work with the Playwright lockstep ([deployment runbook](deployment.md#9-not-built-yet)).
...
**The host-side tooling does not exist, by design.** `./flow release` builds and inspects artifacts on a developer machine ([deployment runbook §8](deployment.md#8-flow-release-the-developer-side-tooling)); `./flow` has no `backup`, `restore` or `deploy` command and will not. The Apache rules the design requires — the `/api` and `/up` carve-out, the maintenance arm, the SPA fallback, the private-path denials — are not yet in `public/.htaccess`. Those arrive in the implementation phase, together with the composed-rule verification noted in section 4a.
