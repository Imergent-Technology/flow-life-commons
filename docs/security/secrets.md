...
**The production environment template is `apps/platform/.env.production.example`.** The development file's two most dangerous values — `APP_DEBUG=true` and `IDENTITY_COMPROMISED_PASSWORD_CHECK=none` — reach production by being copied, which is the accident `security:production-check` catches *after the fact*; the template makes copying the right file easier than copying the wrong one. It holds **no secret**: `APP_KEY` and the three database values are blank for the operator to supply on the host, a test fails if any of them gains a value, and the development credentials are asserted absent. It is committed documentation and is **never shipped in a release artifact** — the release validator refuses any `.env*` file in one. Outbound mail in it is `log`, the documented deferral, not a configuration.
